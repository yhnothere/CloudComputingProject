...
import { getBean } from "@/lib/dynamodb";
...
export default async function BeanDetailPage({ params }: {  params: { id: string };}) {
    ...
    const bean = (await getBean(id)) as CoffeeBean | undefined;

    if (!bean) {
        return <p>Bean not found</p>;
    }
    ...
    return (
        ...
        <main className="min-h-screen bg-stone-50 pt-16">
            ...
            <div>
                <div
                className={`relative rounded-3xl overflow-hidden bg-gradient-to-br ${grad} aspect-square flex items-center justify-center mb-6`}
                >
                    <Image
                ...
                <ScoreBar label="Acidity" score={bean.acidity_score} color="bg-amber-400" />
                <ScoreBar label="Bitterness" score={bean.bitterness_score} color="bg-stone-500" />
                <ScoreBar label="Body" score={bean.body_score} color="bg-amber-700" />
                </div>
            </div>
